...
export default class PriceMenu extends Component {
    ...
    constructor(props){
      ...
      let priceRange = {id: 1, id: 2, id: 3, id: 4};

      handleClick = () => { 
        console.log("Button clicked...")
        let buttonText = this.state.buttonText === "check_box_outline_blank"  && priceRange.id ? "check_box" : "check_box_outline_blank"
        ...
        return (
            <div className="hidden md:block pt-3 w-full">
                <ul className="pl-0 pb-3">
                    ...
                    <span className="material-icons-outlined mr-3" key='1' onClick={this.handleClick}>{this.state.buttonText}</span> 
                        Lower than <span className="ml-1 font-semibold text-sm material-icons-outlined">attach_money</span>20
                    </li>
                    <li className="flex flex-row items-center space-around  text-2xl text-black mb-4">
                        <span className="material-icons-outlined mr-3" key='2'  onClick={this.handleClick}>{this.state.buttonText}</span> 
                    ...
                    <li className="flex flex-row items-center space-around  text-2xl text-black mb-4">
                        <span className="material-icons-outlined mr-3" key={3}  onClick={this.handleClick}>{this.state.buttonText}</span> 
                    ...
                    <li className="flex flex-row items-center space-around  text-2xl text-black mb-4">
                        <span className="material-icons-outlined mr-3" key={4}  onClick={this.handleClick}>{this.state.buttonText}</span> 
